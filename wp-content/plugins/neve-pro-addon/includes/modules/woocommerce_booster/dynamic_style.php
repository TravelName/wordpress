		...
		return [
			...
			[
				...
				'activate_callback' => function () {
					...
					$is_the_page_contains_comparison_table = is_a( $post, 'WP_Post' ) && ( Mods::get( Module::MODS_COMPARISON_TABLE_TABLE_PAGE_ID ) === $post->ID );

					// TODO: review the condition in the next versions. ( to avoid return true if popup enabled only. )
					return ( $is_popup_enabled || $is_the_page_contains_comparison_table );
				},
			],
			[
