...
class Catalog {
	...
	public function __construct() {
		add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'render_catalog_compare_button' ), 13 );
	}
	...
	public function render_catalog_compare_button() {
		...
		$wishlist_button_position  = Mods::get( self::WISHLIST_BUTTON_POSITION, 'none' );

		if ( $compare_checkbox_position === 'top' && $wishlist_button_position === 'top' ) {
			$compare_checkbox_position = 'top-middle';
		} elseif ( $compare_checkbox_position === 'bottom' && $wishlist_button_position === 'bottom' ) {
			$compare_checkbox_position = 'bottom-middle';
		}

		?>
		<div class="nv-ct-compare-btn-wrap <?php echo esc_attr( $compare_checkbox_position ); ?>">
